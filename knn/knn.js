...
function compress(data, rowSegmentLength, columnSegmentLength) {
  const compressed = [];
  const rowLength = Math.sqrt(data.length),
    columnLength = Math.sqrt(data.length);
  const grid = createGrid(data, rowLength);

  // Iterate over the array with a square window
  for (
    let rowSegment = 0;
    rowSegment < rowLength;
    rowSegment += rowSegmentLength
  ) {
    for (
      let columnSegment = 0;
      columnSegment < columnLength;
      columnSegment += columnSegmentLength
    ) {
      ...
      for (let x = rowSegment; x < rowSegment + rowSegmentLength; x++) {
        for (
          let y = columnSegment;
          y < columnSegment + columnSegmentLength;
          y++
        ) {
          pixel.push(grid[x][y]);
        }
